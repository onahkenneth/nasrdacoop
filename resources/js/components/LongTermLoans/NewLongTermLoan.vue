    ...
    <div class="row">
      <div class="col-md-6">
        ...
        <div class="card text-white bg-info">
          <div class="card-body">
            <h5 class="card-title">Savings Balance: &#8358; {{ savings ? savings.bal : 0 | number_format }}</h5>
          </div>
        </div>
...
export default {
  ...
  methods: {
    calMaxLoan: function(event) {
      ...
      this.max_loan_amount = event.target.value == "36" ? savings_bal  * 3 : savings_bal  * 2;
    ...
    submitForm: function() {
      ...
      axios
        .post(`members/post-new-long-loan/${this.staff.ippis}`, this.loan)
        .then(res => {
          ...
          var baseUrl =
            ...
            window.location.href = baseUrl + `/public/members/${this.staff.ippis}/long-term`;
            // getUrl.pathname.split("/")[0];
            // window.location.href = baseUrl + `/members/${this.staff.ippis}/long-term`;
        })
        .catch(e => {
          // console.log(e);
  ...
  created() {
    ...
    this.loan.ippis = this.staff.ippis;
      axios
        .get(`members/${this.staff.ippis}/new-long-loan`, this.loan)
        .then(res => {
          // console.log(res.data);
