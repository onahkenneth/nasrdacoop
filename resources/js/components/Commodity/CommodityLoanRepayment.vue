...
export default {
  ...
  methods: {
    submitForm: function() {
      ...
      if (this.repayment.repayment_mode == 'savings') {
        ...
        if (this.repayment.total_amount > this.last_commodity_loan_payment.bal) {
      ...
      axios
        .post(`members/commodity-loan-repayment/${this.staff.ippis}`, this.repayment)
        .then(res => {
  ...
  created() {
    this.repayment.ippis = this.staff.ippis;
      axios
        .get(`members/commodity-loan-repayment/${this.staff.ippis}`, this.loan)
        .then(res => {
          console.log(res.data);
